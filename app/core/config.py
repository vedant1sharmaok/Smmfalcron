...
CRITICAL: No lru_cache on Settings().
On Render, DATABASE_URL is injected at runtime. If Settings() is cached
at import time, it reads the localhost default → ConnectionRefusedError.
"""
...
class Settings(BaseSettings):
    ...
    # ── Admin Bot ─────────────────────────────────────────────────────────────
    admin_bot_token:           str = ""   # separate bot token for admin panel
    admin_bot_webhook_secret:  str = ""
    admin_telegram_ids:        str = ""   # comma-separated Telegram IDs allowed

    # ── Support ───────────────────────────────────────────────────────────────
    support_username: str = ""            # @username shown in support menu
...
def get_settings() -> Settings:
    """Fresh Settings() every call — no lru_cache (see module docstring)."""
    return Settings()


# Module-level singleton — created once after Render injects env vars
settings = Settings()
