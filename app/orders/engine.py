...
async def create_order(
    ...
    # ── Step 13: Risk/fraud check ────────────────────────────────────────────────
    try:
        from app.risk.engine import evaluate_order_risk
        risk = await evaluate_order_risk(
            user_id=user_id,
            amount_inr=float(final_price),
            service_id=service_id,
        )
        if risk.is_blocked():
            raise OrderError(
                detail=f"Order blocked by risk engine: {risk.reasons}",
                user_message="Your order could not be processed. Please contact support if you believe this is an error.",
            )
    except OrderError:
        raise
    except Exception:
        pass  # Risk engine errors never block legitimate orders
